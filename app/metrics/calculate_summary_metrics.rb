module Metrics
	class CalculateSummaryMetrics
		...
		def calculate_single_dgId_metrics(dgId)
			detGroupMetrics = @video.det_group_metrics.where(det_group_id: dgId).order_by([:frame_number, :asc])

			# get resolutions to create metrics
			resolutions = States::SummaryResolutions.new.getFrameCounters(@video.detection_frame_rate)
			...
			frameNumTimes = Hash[detGroupMetrics.pluck(:frame_number, :frame_time)]
			...
			sortedFrameNums.each do |frameNum|
				...
				dgvf = detGroupMetrics.where(frame_number: frameNum).first

				# BEGIN - enter value for reach resolution
				resolutions.each do |t, res|
				...
				# END - enter value for reach resolution

				# BEGIN - average results and reset counters
				resolutions.each do |t, res|
					if res[:frame_counter] == res[:num_of_frames]
						brand_effectiveness[t] /= res[:frame_counter]
						det_group_crowding[t] /= res[:frame_counter]
						visual_saliency[t] /= res[:frame_counter]
						timing_effectiveness[t] /= res[:frame_counter]
						spatial_effectiveness[t] /= res[:frame_counter]
						#detections_count[t] = detections_count[t] --> raw count, not average
						quadrants[t].each do |k,v|
							quadrants[t][k] = v/res[:frame_counter]
						end
		...
		def calculate_new
			# sanity check
			videoDetGroupIds = @video.det_group_metrics.pluck(:det_group_id)
			raise "Video has no det group metrics saved" if videoDetGroupIds.count == 0
			@detGroupIds.each do |dgId|
				raise "Video is not evaluated against det group #{dgId}" if not videoDetGroupIds.include?(dgId)
			end

			# calculate for each dgId
			@detGroupIds.each do |dgId|
				calculate_single_dgId_metrics(dgId)
			end
